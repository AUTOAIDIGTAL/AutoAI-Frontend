...
import { ScheduledTaskContext } from "@/app/(pages)/(dashboard)/scheduled-tasks/scheduleTasksContext";
import { WorkOrderModal } from "@/app/(pages)/(dashboard)/scheduled-tasks/explore/WorkOrderModal";
...
const ParentComponent = () => {
	...
	return (
		<>
			<TableST handleShow={handleShow} workOrders={workOrders} setWorkOrderId={setWorkOrderId} />
			{<WorkOrderModal show={show} handleClose={handleClose} workOrderId={workOrderId} />}
		</>
	);
};

const TableST = ({ handleShow, workOrders, setWorkOrderId }) => {
		...
		<Table hover responsive>
			...
			<tbody>
				{workOrders?.map((_, index) => (
					<tr key={index} onClick={() => { handleShow(); setWorkOrderId(_?._id) }} role="button">
						<td>
							<div className="text-dark">{_?.vehicle?.regPlate}</div>
						</td>
						<td>
							<div className="text-muted">{_?.customer?.name}</div>
						</td>
						<td>
						...
						<td>
							<div className="fs-6 fw-medium d-inline-flex align-items-center gap-1 rounded-5 bg-gray-100 py-1 px-2">
								<div className="text-primary">{_?.inprogressCount}</div>
								<div className="text-primary">/</div>
								<div className="text-muted">{_?.totalJobs}</div>
							</div>
						</td>
						<td>
							<MultipleAvatars />
